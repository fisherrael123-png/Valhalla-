...
def validate_status_service():
    ...
    require(
        "`current status`" in router or "current status" in router,
    ...
    require(
        "`status`" in command_reference and "system state" in command_reference and "knowledge-base state" in command_reference,
...
def validate_remove_kb_service():
    ...
    require("is not deleted" in command_reference, "command reference must state remove_kb does not delete files")
...
def validate_register_existing_kb_service():
    ...
    for token in (
        ...
        "Wiki/Wiki_<knowledge-base-name>/",
        "does not create",
        "modify files inside the knowledge base",
        "resource_registry.yaml",
...
def validate_sync_resource_usage_service():
    ...
    for token in (
        ...
        "`kb_name`, `entity_id`, and `entity_file`",
        "Old string-path form",
        "Old relative-path form",
        "Old `kb` field form",
...
def require_inline_resource_usage_sync(
    ...
    if operation_name == "lint":
        ...
        require(
            "non-usage field in resource_registry.yaml" in separate_confirmation,
...
def require_ingest_inline_usage_sync():
    ...
    for token in (
        "This operation must synchronize usage in `resource_registry.yaml` and `resource_registry.md`",
        ...
        "Do not write legacy usage",
        "solely from `resource_registry.yaml`",
        "set `next_operation: null`",
    ):
...
def validate_resource_usage_canonical_workflows():
    ...
    for relative_path in (
        ...
        text = read_text(relative_path)
        require(
            "Do not modify `usage.referenced_by`" in text or "Do not modify `resource_registry.yaml`" in text,
...
def validate_ingest_entity_numbering_rule():
    ...
    for token in (
        "Calculate `entity_id` only from local state",
        ".registry/machine/entity_registry.yaml",
        "entities/ent_*.md",
        "Do not derive the next `entity_id` from `resource_registry.yaml`",
        "usage",
    ):
...
def validate_fuse_kbs_service():
    ...
    for token in (
        "`fuse_kbs`",
        "Fuse multiple knowledge bases",
        "fusing every knowledge base in the current root is not",
    ...
    for token in (
        ...
        "Fusing every knowledge base under the current root is not supported",
        "entity",
        "relationship",
        "conversation_entity",
        "engineering_entity",
        ...
        "entity_id_map",
        "excluded_but_used_resources",
        "Source KBs remain read-only throughout",
        "Positive set",
        "Evidence set",
        "Exclusion set",
        "merge_content",
...
def validate_migrate_kb_service():
    ...
    for token in (
        ...
        "target root is always the current root",
        "new name",
    ...
    for token in (
        "inspect",
        "fix",
        "The target root is always the current root",
        "root2:ai-engineering",
        "new name",
        "source root remains read-only throughout",
        "resource_id_map",
        "reuse_current",
        "create_new",
...
def validate_templates_for_human_projection():
    ...
    for relative_path in human_templates:
        ...
        require("human-readable" in text, f"{relative_path} must describe human-readable projection")
